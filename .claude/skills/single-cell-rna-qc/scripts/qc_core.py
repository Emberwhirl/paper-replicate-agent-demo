...
def calculate_qc_metrics(adata, inplace=True):
    """
    Calculate QC metrics for single-cell RNA-seq data.

    Uses uppercase pattern matching for cross-species compatibility:
    - Mitochondrial: MT- (works for human MT-, mouse mt-/Mt-)
    - Ribosomal: RPS/RPL (works for human RPS/RPL, mouse Rps/Rpl)
    - Hemoglobin: HB (excluding HBP, works for human HB, mouse Hb)
    ...
    # Identify gene categories using uppercase pattern matching for cross-species compatibility
    # Mitochondrial genes
